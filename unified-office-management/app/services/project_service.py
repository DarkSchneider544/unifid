...
class ProjectService:
    ...
    async def create_project(
        ...
        # Validate only Team Leads can create projects
        if requested_by.role != UserRole.TEAM_LEAD and not requested_by.is_team_lead:
            if requested_by.role not in [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.MANAGER]:
        ...
        requester_in_members = any(
            str(m.user_id) == str(requested_by.id) 
    ...
    async def list_projects(
        ...
        if requested_by_id:
            query = query.where(Project.requested_by_id == requested_by_id)
            count_query = count_query.where(Project.requested_by_id == requested_by_id)
